...
router = APIRouter(prefix="/api/v2/upload", tags=["File Upload(v2)"])


@router.post(
    "/nc", status_code=201, response_model=FileCreateResponse, summary="NC 파일 업로드"
)
async def upload_nc_file(
    ...
    nc_filename = nc_file.filename
    project = await project_service.get_project_by_id(project_id)
    file_id = await file_service.process_upload(nc_file)
    await project_service.nc_upload(project, workplan_id, file_id, nc_filename)
...
async def upload_tdms_path_log(
    ...
    log_file: Optional[UploadFile] = File(
        None, description="업로드할 TDMS 확장 로그 파일"
    ),
    project_service: AssetProjectService = Depends(get_asset_project_service),
    file_service: FileService = Depends(get_file_service),
):
    ...
    file_id = await file_service.process_upload(log_file, {"nc_code": nc_code_id})
    log_filename = log_file.filename
    await project_service.tdms_upload(
        project,
        workplan_id,
        file_id,
        log_filename,
...
@router.post(
    ...
    status_code=201,
    response_model=FileCreateResponse,
    summary="Virtual Machine 파일 업로드",
)
async def upload_virtual_machine_file(
    ...
    project = await project_service.get_project_by_id(project_id)
    await file_service.file_exist(nc_code_id)
    file_id = await file_service.process_upload(vm_file, {"nc_code": nc_code_id})
    file_name = vm_file.filename
    await project_service.vm_upload(project, workplan_id, file_id, file_name)
    return FileCreateResponse(file_id=file_id)
...
async def upload_cam_file(
    ...
    """
    project = await project_service.get_project_by_id(project_id)
    await project_service.add_cam_to_project(project, cam_type, cam_json, mapping_json)
    return

...
export default function MachineList({ onSelect, disabled }: MachineListProps) {
  ...
  useEffect(() => {
    fetchDevices(); // mount 시 1회 호출
    const intervalId = setInterval(fetchDevices, 60000); // 1분마다 polling
    ...
    <div>
      ...
      >
        {error ? (
          ...
          <div className="text-gray-400 text-sm text-center py-2">연결된 장비가 없습니다.</div>
        ) : (
          devices.map((dev) => (
            <div
              key={dev.id}

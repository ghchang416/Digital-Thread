...
KNOWN_STOCK_CODES: set[str] = {str(item["code"]) for item in STOCK_ITEMS}


def lookup_stock_code(name: str | None) -> str | None:
    if not name:
        return None
    code = STOCK_CODE_BY_NAME.get(name)
    return str(code) if code is not None else None


def is_known_stock_code(code: str | int | None) -> bool:
    if code is None:
        return False
    return str(code) in KNOWN_STOCK_CODES
